...
const total_order_ingredients_products = (order_ingredients_products) => {
	...
	for (ingredient_product in order_ingredients_products) {
		order_ingredients_products_total += parseFloat(
			order_ingredients_products[ingredient_product].price
		);
	}
...
class Order extends Component {
	...
	componentDidMount() {
		this.setState({
			order_ingredients_products: JSON.parse(
				localStorage.getItem("checkoutList")
			),
			total: Number(
				localStorage
					.getItem("checkoutTotal")
					.substring(1, localStorage.getItem("checkoutTotal").length - 1)
			),
		});
	}
	...
	componentDidUpdate(prevProps, prevState) {
		if (
			prevState.order_ingredients_products !==
			this.state.order_ingredients_products
		) {
			this.setState({
				show: true,
			});
		...
		return (
			<>
				...
				<Container className="checkout-container2" fluid>
					<Row>
						<Col>
							<OrderDetailsH6>Ingredient/Product</OrderDetailsH6>
							{/* <h6><b>Ingredient/Product</b></h6> */}
						</Col>
						<Col>
							<OrderDetailsH6>Price</OrderDetailsH6>
						</Col>
					</Row>
					{this.state.show ? (
					...
					<Row>
						<Col>
							<h5>Total: {this.state.show ? this.state.total : ""} €</h5>
						</Col>
					</Row>
					<hr />
					<center>
